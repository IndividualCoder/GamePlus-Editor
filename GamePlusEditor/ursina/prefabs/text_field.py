...
class TextField(Entity):
    def __init__(self, max_lines=9999, **kwargs):
        ...
        if "text_scale" in kwargs:
            self.text_entity = Text(parent=self.scroll_parent, start_tag='☾', end_tag='☽', font=self.font, text='', line_height=self.line_height, origin=(-.5, .5),scale = kwargs["text_scale"])
        else:
...
# /        self.line_numbers_background = Entity(parent=self.scroll_parent,model = "cube", origin=(.5,.5), x=-.04, color=color.black, enabled=False)
        ...
        if "text_scale" in kwargs:
            self.cursor_parent = Entity(parent=self.scroll_parent, scale=(self.character_width*kwargs['text_scale'], -1*Text.size*kwargs['text_scale']),y = 0*kwargs['cursor_y'])
        else:
        ...
        self.active = True
        self.highlight_color = color.color(120,1,1,.2)
        self.text = ''
        ...
        self.shortcuts = {
            ...
            'scroll_up':        ('scroll up','shift+up arrow'),
            'scroll_down':      ('scroll down','shift+down arrow'),
            'cut':              ('ctrl+x',),
            'copy':             ('ctrl+c',),
            'paste':            ('ctrl+v',),
            'select_all':       ('ctrl+a',),
    ...
    def input(self, key):
        ...
        if self.register_mouse_input and key == 'left mouse down':
            if mouse.hovered_entity == self.bg:
                self.active = True
                return 
            elif mouse.hovered_entity != self.bg and not held_keys["shift"]:
                self.active = False
                return 

            # elif mouse.hovered_entity == self.bg and held_keys["shift"]:
    ...
    def render(self):
        ...
        if not hasattr(self.text_entity, 'raw_text') or self._prev_text != text or self.scroll != self._prev_scroll:
            ...
            self.line_numbers.text = ('     \n'*self.scroll) + '\n'.join([str(e + self.scroll) for e in range(min(len(lines), self.max_lines))])
