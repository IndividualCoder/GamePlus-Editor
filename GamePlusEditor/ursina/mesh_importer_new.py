...
def obj_to_ursinamesh(path=application.compressed_models_folder, outpath=application.compressed_models_folder, name='*', return_mesh=True, save_to_file=False, delete_obj=False):
    ...
    for f in path.glob(f'**/{name}.obj'):
        ...
        for l in lines:
            if l.startswith('v '):
                    ...
                    with open(mtl_file_name) as mtl_file:
                        ...
                        for i in range(len(mtl_data)-1):
                            if mtl_data[i].startswith('newmtl '):
                                ...
                                materials[material_name] = {'color':None, 'texture':None}

                                for j in range(i+1, min(i+10, len(mtl_data))):
                                    if mtl_data[j].startswith('newmtl'):
                                        break
                                    if mtl_data[j].startswith('Kd '):
                                        # print('found color')
                                        material_color = [float(e) for e in mtl_data[j].strip()[3:].split(' ')]
                                        materials[material_name]['color'] = *material_color, 1
                                        continue
                                    if mtl_data[j].startswith('map_Kd '):
                                        materials[material_name]['texture'] = mtl_data[j][7:].strip()
                                        # print('found texture:', mtl_data[j][7:])
                                        continue
                ...
                if material_name in materials:
                    current_color = materials[material_name]['color']
                    # print('------',current_texture, materials[material_name]['texture'])

            elif l.startswith('o ') and mtl_data:
                # if current_texture != materials[material_name]['texture'] and tris:  # if we change texture, make a new Mesh
                current_texture = materials[material_name]['texture']
                textures.append(current_texture)
                vertices = [verts[t] for t in tris]

                # print('NEW MAEH')
                m = Mesh(vertices=vertices,normals=normals, uvs=[uvs[uid] for uid in uv_indices], colors=vertex_colors)
                meshes.append(m)
                offset += len(tris)
                # verts.clear()
                tris.clear()
        ...
        if return_mesh:
            if not meshes:  # return one mesh
                return Mesh(vertices=[verts[t] for t in tris], normals=normals, uvs=[uvs[uid] for uid in uv_indices], colors=vertex_colors)

            # return multiple meshes with different textures
            from GamePlusEditor.ursina import Entity
            mesh_parent = Entity()
            for i, m in enumerate(meshes):
                print(i, textures[i])
                Entity(parent=mesh_parent, model=m, texture=textures[i])
...
if __name__ == '__main__':
    ...
    # application.asset_folder = Path('/home/poke/programming/ursina/samples/')
    # Entity(model='ambulance', x=-3)
    # # application.asset_folder = Path(Path('.').parent / 'samples' / 'models').resolve()
    # # print('----------', application.asset_folder)
    # application.asset_folder = Path('/home/poke/Downloads/Zazen')
    # e = Entity(model='Machi_StageNum_366.obj')

    application.asset_folder = Path('/home/poke/Downloads/')
    e = Entity(model='lamp', scale=.1, double_sided=True)
    # for c in e.model.children:
    #     c.texture='StoneWall.png'
