...
class PositionGizmo(Entity):
    def __init__(self, Entity, Snapping,OnDrag, **kwargs):
        ...
        self.GizmoX = Draggable(parent = self,lock = (0,1,1),scale = (1,1,1),rotation = (90,90,0),color = color.red,model = f"PositionGizmo.obj",collider = "mesh",double_sided = True,step = Snapping,position = Entity.position)
        self.GizmoY = Draggable(parent = self,lock = (1,0,1),scale = (1,1,1),rotation = (0,90,0),color = color.blue,model = f"Models/PositionGizmo.obj",collider = "mesh",double_sided = True,step = Snapping,position = Entity.position)
        self.GizmoZ = Draggable(parent = self,lock = (1,0,0),plane_direction = (0,1,0),rotation = (90,90,90),scale = (1,1,1),color = color.green,model = f"Models/PositionGizmo.obj",collider = "mesh",double_sided = True,step = Snapping,position = Entity.position)
...
if __name__ == "__main__":
    ...
    a = Entity(model = "cube")
    ed = PositionGizmo(a,0,lambda: ...);ed.SetUp()
    def input(key):
        if key == "1":
            ed.Snapping = 1
        elif key == "2":
